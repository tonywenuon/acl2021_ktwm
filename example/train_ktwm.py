...
from models.ktwm import KTWMModel
...
#from vspgt_data_reader_fact_mask_new import DataSet
...
class KTWM:
    def __init__(self, args):
        ...
        self.transformer_model = KTWMModel(args=args,
        ...
        if args.data_set == 'full_reduced':
            exp_name = args.data_set + '_' + args.exp_name + '_' + str(args.fact_number)
    ...
    def compile_new_model(self, pad_id):
        _model, emb_fn = self.transformer_model.get_model(pad_id)
        _model.compile(
                       optimizer=keras.optimizers.Adam(lr=self.args.lr),
                       loss = None,
                       #loss = keras.losses.sparse_categorical_crossentropy,
                      )
        return _model, emb_fn
    ...
    def train(self):
        ...
        train_generator = ds.data_generator('train', 'ktwm')
        valid_generator = ds.data_generator('valid', 'ktwm')
        ...
        if os.path.exists(self.model_path):
            ...
            model, _ = self.compile_new_model(ds.pad_id)
    ...
    def test(self):
        ds = DataSet(args)
        test_generator = ds.data_generator('test', 'ktwm')
        ...
        for batch_index, ([src_input, tar_input, tar_loss_input, facts_input, fact_switches, is_train, _], _) in enumerate(test_generator):
            ...
            for t in range(1, tar_length):
                _, preds = model.predict([src_input, np.asarray(results), tar_loss_input, facts_input, fact_switches, is_train]) 
            ...
            def output_results(outputs, outobj):
                for result in outputs:
                    ...
                    for _id in result:
                        ...
                        #if _id != ds.pad_id:
                            seq.append(ds.tar_id_tokens.get(_id, self.config.UNK_TOKEN))
                    write_line = ' '.join(seq)
                ...
                for i in range(beam_size):
                    ...
                    for j in range(beam_size-1, -1, -1):
                        ...
                        for bs_idx, _id in enumerate(top_index):
                            ...
                            # get OOV id
                            token = ds.tar_id_tokens.get(int(_id), self.config.UNK_TOKEN)
