...
class TransformerCoordinateEmbedding(Layer):
    ...
    def get_config(self):
        config = super().get_config()
        config['max_sequence_length'] = self.max_seq_length
    ...
    def build(self, input_shape):
        ...
        self.word_position_embeddings = self.add_weight(
            shape=(self.max_seq_length, d_model),
            initializer='uniform',
...
class SrcFactTransformerCoordinateEmbedding(Layer):
    """
    Represents trainable positional embeddings for the Transformer model:

    1. word position embeddings - one for each position in the sequence.
    2. depth embeddings - one for each block of the model
    3. src_fact embeddings - one for src and facts order
    ...
    def __init__(self, 
                 max_sequence_length: int, 
                 max_transformer_depth: int, 
                 max_src_facts_depth: int, 
                 **kwargs):
        self.max_seq_length = max_sequence_length 
        self.max_depth = max_transformer_depth
        self.max_sf_depth = max_src_facts_depth
        super().__init__(**kwargs)
    ...
    def get_config(self):
        ...
        config['max_sequence_length'] = self.max_seq_length
        config['max_transformer_depth'] = self.max_depth
        config['max_src_facts_depth'] = self.max_sf_depth 
        # 
        return config
    ...
    def build(self, input_shape):
        ...
        self.depth_embeddings = self.add_weight(
            ...
            name='depth_position_embeddings',
            trainable=True)
        # src and facts order embeddings
        self.sf_depth_embeddings = self.add_weight(
            shape=(self.max_sf_depth, d_model),
            initializer='uniform',
            name='sf_depth_position_embeddings',
            trainable=True)
...
get_custom_objects().update({
    ...
    'SrcFactTransformerCoordinateEmbedding': SrcFactTransformerCoordinateEmbedding,
